...
export default function ChatInterface() {
  ...
  // Setup event listener for suggested questions
  useEffect(() => {
    const handleSuggestedQuestion = (event: CustomEvent) => {
      if (event.detail) {
        ...
        chatMutation.mutate(event.detail);
      }
    };
  ...
  const chatMutation = useMutation({
    mutationFn: async (message: string) => {
      return apiRequest<any>("/api/chat", {
        method: "POST",
        data: { message },
      });
    },
      ...
      <div className="border-t border-slate-200 p-4 bg-white">
        <form onSubmit={handleSubmit} className="flex gap-2">
          <Input
            ...
            placeholder="Ask a question about product management, business strategy, or growth..."
            className="flex-grow"
            disabled={isTyping}
          />
          <Button 
